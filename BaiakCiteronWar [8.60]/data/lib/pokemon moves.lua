...
if cd >= 0 or cd2 >= 0 or cd3 >= 0 then                                                         --alterado v1.7
   if not isInArray({"Aromateraphy", "Emergency Call", "Magical Leaf", "Sunny Day", "Safeguard", "Hitmonmind", "Rain Dance"}, spell) and getPlayerStorageValue(cid, 21100) <= -1 then
...
if isCreature(cid) then
   ...
   doAreaCombatHealth(cid, BUGDAMAGE, area, 0, 0, 0, eff)
   doAreaCombatHealth(cid, BUGDAMAGE, area, whirl3, -min, -max, 84)
end
end
...
local t = {
[0] = {110, {x=p.x, y=p.y-(a+1), z=p.z}},           --alterado v1.4
[1] = {110, {x=p.x+(a+1), y=p.y, z=p.z}},
[2] = {110, {x=p.x, y=p.y+(a+1), z=p.z}},
[3] = {110, {x=p.x-(a+1), y=p.y, z=p.z}}
}   
...
elseif spell == "Venom Sting" then
       
       doSendDistanceShoot(getThingPosWithDebug(cid), getThingPosWithDebug(target), 15)
       doDanoInTargetWithDelay(cid, target, POISONDAMAGE, min, max, 84) --alterado v1.7
...
doMoveInArea2(cid, 104, thunderr, ELECTRICDAMAGE, min, max, spell, ret)
...
for rocks = 1, 42 do
    addEvent(fall, rocks*35, cid, master, ELECTRICDAMAGE, 41, 104)
end
end
...
elseif spell == "Restore" or spell == "Selfheal" or spell == "Aquaring" then
...
elseif spell == "Fly" then
	...
	doDanoWithProtect(cid, FLYINGDAMAGE, getThingPosWithDebug(cid), selfArea2, min, max, 42) 	
...
elseif spell == "Ilusion Mind" then
...
elseif isInArray({"Pursuit", "Light Shock", "ExtremeSpeed", "U-Turn", "Shell Attack"}, spell) then

local atk = {
["Pursuit"] = {17, DARKDAMAGE},
["Light Shock"] = {41, ELECTRICDAMAGE},
...
local life = getCreatureHealth(target)
	...
	doSendMagicEffect(getThingPosWithDebug(cid), 84)
	if newlife >= 1 then
	doCreatureAddHealth(cid, newlife)
	doSendAnimatedText(getThingPosWithDebug(cid), "+"..newlife.."", 32)
	end  
...
elseif spell == "Iron Slice" then

local function sendStickEff(cid, dir)
    if not isCreature(cid) then return true end
       doAreaCombatHealth(cid, STEELDAMAGE, getPosByDir(getThingPosWithDebug(cid), dir), 0, -min, -max, 77)
	end
...
elseif spell == "Powder Snow" then
	...
	doDanoWithProtect(cid, ICEDAMAGE, getThingPosWithDebug(cid), selfArea2, min, max, 205) 
...
elseif spell == "shadow" then
    
local pos = getThingPosWithDebug(cid)

doMoveInArea2(cid, EFEITO, shadow, ELEMENTO, 0, 0, spell)
addEvent(doDanoWithProtect, math.random(100, 400), cid, ELEMENTO, pos, shadow, -min, -max, 0)
...
["Electivire"] =  {207, ELECTRICDAMAGE, 1462},
["Magmortar"] =  {5, FIREDAMAGE},
["Shiny Electivire"] =  {207, ELECTRICDAMAGE, 1462},
